...
import {
  BRANCHES, DEFAULT_CUSTOMIZATION, LEGACY_ORDERING_STORAGE_KEY, ORDERING_STORAGE_KEY, PRODUCTS, branchById, calculateTotals,
...
const ORDER_TYPES: Array<{ type: OrderType; description: string; icon: typeof Store }> = [
  ...
  { type: "Delivery", description: "Delivered from a nearby location.", icon: Bike },
...
export function RestaurantOrderingShowcase() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      const stored = window.localStorage.getItem(ORDERING_STORAGE_KEY) ?? window.localStorage.getItem(LEGACY_ORDERING_STORAGE_KEY);
      const loaded = loadOrderingState(stored);
      window.localStorage.removeItem(LEGACY_ORDERING_STORAGE_KEY);
      setState(loaded);
  ...
  function applyPromo() {
    ...
    if (!["WELCOME10", "FREESIDE"].includes(code)) return setPromoMessage({ tone: "error", text: "That promotion code is not valid." });
  ...
  function resetDemo() { window.localStorage.removeItem(ORDERING_STORAGE_KEY); window.localStorage.removeItem(LEGACY_ORDERING_STORAGE_KEY); setState(createInitialOrderingState()); setPromoInput(""); setPromoMessage(null); setResetOpen(false); navigate("Landing"); setNotice("Data restored to its original state."); }

  return <div className={styles.demo} data-ready={ready}>
    <ConceptBar />
    <header className={styles.header}>
      <button className={styles.brand} type="button" onClick={() => navigate(state.orderType && state.branchId ? "Menu" : "Landing")} aria-label="ILBATECH home"><span><UtensilsCrossed /></span><strong>ILBATECH</strong></button>
      ...
      <div className={styles.headerActions}><button className={styles.reset} type="button" aria-label="Reset data" title="Reset Data" onClick={() => setResetOpen(true)}><RefreshCcw /><span>Reset</span></button><button className={styles.cartButton} type="button" onClick={() => navigate("Cart")}><ShoppingBag /><span>Cart</span>{cartCount > 0 && <b>{cartCount}</b>}</button></div>
    </header>
...
function CollectionScreen({ title, eyebrow, empty, products, ...actions }: ProductActions & { title: string; eyebrow: string; empty: string; products: Product[] }) { return <section><PageIntro eyebrow={eyebrow} title={title} text="Your saved menu items are ready when you are." />{products.length ? <div className={styles.productGrid}>{products.map((product) => <ProductCard product={product} {...actions} key={product.id} />)}</div> : <div className={styles.empty}><Heart /><h2>No favorites yet</h2><p>{empty}</p></div>}</section>; }
...
function CheckoutScreen({ state, totals, step, setStep, setAccountMode, placeOrder, back }: { state: OrderingState; totals: ReturnType<typeof calculateTotals>; step: number; setStep: (step: number) => void; setAccountMode: (mode: AccountMode) => void; placeOrder: (customer: { name: string; phone: string; email: string }, payment: PaymentMethod, fulfillmentNote: string) => void; back: () => void }) {
  ...
  function submit() { if (payment === "Card" && (card.replace(/\s/g, "") !== "4242424242424242" || !expiry || cvv !== "123")) { setError("Use test card 4242 4242 4242 4242, any future expiry, and CVV 123."); return; } setError(""); placeOrder({ name, phone, email }, payment, note || `${state.orderType} at ${branch.name}`); }
  return <section><PageIntro back={step === 1 ? back : () => setStep(step - 1)} eyebrow="Checkout" title="Almost there" text="Demo payment — no charge will be made, and payment details are not stored." /><div className={styles.checkoutLayout}><div className={styles.checkoutCard}><ol className={styles.checkoutSteps}>{["Fulfillment", "Your details", "Payment"].map((label, index) => <li data-active={step === index + 1} data-done={step > index + 1} key={label}><span>{step > index + 1 ? <Check /> : index + 1}</span>{label}</li>)}</ol>{error && <p className={styles.formError} role="alert">{error}</p>}{step === 1 && <div className={styles.checkoutSection}><span className={styles.eyebrow}>Step 1</span><h2>{state.orderType} details</h2><div className={styles.branchSummary}><Store /><span><strong>{branch.name}</strong><small>{branch.address} · {branch.prepMinutes}</small></span></div>{state.orderType === "Delivery" && <TextField label="Delivery address" value={note} setValue={setNote} placeholder="Apartment, street, city" />}{state.orderType === "Pickup" && <TextField label="Pickup instructions (optional)" value={note} setValue={setNote} placeholder="Anything the team should know?" />}{state.orderType === "Dine In" && <TextField label="Table number" value={note} setValue={setNote} placeholder="Example: Table 14" />}{state.orderType === "Drive-Thru" && <TextField label="Vehicle or plate" value={note} setValue={setNote} placeholder="Example: White hatchback" />}<button className={styles.primary} type="button" onClick={next}>Continue to details <ArrowRight /></button></div>}{step === 2 && <div className={styles.checkoutSection}><span className={styles.eyebrow}>Step 2</span><h2>How should we contact you?</h2><div className={styles.accountChoice}><button type="button" data-active={state.accountMode === "Guest"} onClick={() => setAccountMode("Guest")}><UserRound />Guest checkout</button><button type="button" data-active={state.accountMode === "Account"} onClick={() => { setAccountMode("Account"); setName("Noura Karim"); setPhone("+961 70 555 014"); setEmail("noura@example.test"); }}><Heart />Use saved account</button></div><TextField label="Full name" value={name} setValue={setName} /><TextField label="Phone number" value={phone} setValue={setPhone} inputMode="tel" /><TextField label="Email (optional)" value={email} setValue={setEmail} inputMode="email" /><button className={styles.primary} type="button" onClick={next}>Continue to payment <ArrowRight /></button></div>}{step === 3 && <div className={styles.checkoutSection}><span className={styles.eyebrow}>Step 3</span><h2>Select payment</h2><div className={styles.paymentOptions}>{(["Card", "Digital Wallet", "Cash"] as PaymentMethod[]).map((method) => <button type="button" data-active={payment === method} onClick={() => setPayment(method)} key={method}>{method === "Card" ? <CreditCard /> : method === "Digital Wallet" ? <WalletCards /> : <ShoppingBag />}<span><strong>{method === "Digital Wallet" ? "Mobile Wallet" : method}</strong><small>{method === "Card" ? "Simulated card payment" : method === "Digital Wallet" ? "Simulated wallet approval" : "Pay at collection where available"}</small></span><Check /></button>)}</div>{payment === "Card" && <div className={styles.cardFields}><div className={styles.demoCardNote}><CreditCard /><span><strong>Test card</strong><small>Use 4242 4242 4242 4242 · No details are stored.</small></span></div><TextField label="Card number" value={card} setValue={setCard} inputMode="numeric" placeholder="4242 4242 4242 4242" /><div><TextField label="Expiry" value={expiry} setValue={setExpiry} placeholder="12/30" /><TextField label="CVV" value={cvv} setValue={setCvv} inputMode="numeric" placeholder="123" /></div></div>}<button className={styles.primary} type="button" onClick={submit}>Place simulated order · {formatMoney(totals.total)}</button></div>}</div><aside className={styles.summary}><h2>Your total</h2><p>{state.cart.reduce((sum, item) => sum + item.quantity, 0)} items · {state.orderType} from {branch.name}</p><Totals totals={totals} promo={state.promo} /><small><Check /> Demo payment — no charge will be made.</small></aside></div></section>;
}
...
function Tracking({ order, advance, receipt }: { order: Order; advance: () => void; receipt: () => void }) { const stages = trackingStages(order.orderType); const complete = order.statusIndex === stages.length - 1; return <section className={styles.tracking}><PageIntro eyebrow={`${order.id} · ${order.orderType}`} title={complete ? "Order complete" : "We’re on it"} text={complete ? "The order has reached its final status." : "Follow the order from confirmation to completion."} /><div className={styles.trackingCard}><div className={styles.trackingSummary}><PackageCheck /><span><small>Current status</small><strong>{stages[order.statusIndex]}</strong></span><b>{branchById(order.branchId)?.prepMinutes}</b></div><ol>{stages.map((stage, index) => <li data-state={index < order.statusIndex ? "done" : index === order.statusIndex ? "current" : "future"} key={stage}><span>{index <= order.statusIndex ? <Check /> : index + 1}</span><div><strong>{stage}</strong><small>{order.statusTimes[index] ?? "Waiting"}</small></div></li>)}</ol><div className={styles.trackingActions}>{!complete && <button className={styles.secondary} type="button" onClick={advance}>Advance status</button>}{complete && <button className={styles.primary} type="button" onClick={receipt}>Open digital receipt <ReceiptText /></button>}</div></div></section>; }

function Receipt({ order, back }: { order: Order; back: () => void }) { const branch = branchById(order.branchId)!; return <section><PageIntro back={back} eyebrow="Digital receipt" title={order.id} text={`${order.displayDate}, 2026 · 2:32 PM`} /><article className={styles.receipt}><header><span><UtensilsCrossed /></span><div><strong>ILBATECH</strong><small>{branch.name} · {branch.address}</small></div></header><div className={styles.receiptMeta}><span><small>Fulfillment</small><strong>{order.orderType}</strong></span><span><small>Payment</small><strong>{order.paymentMethod === "Digital Wallet" ? "Mobile Wallet" : order.paymentMethod}</strong></span><span><small>Customer</small><strong>{order.customer.name}</strong></span></div><div className={styles.receiptItems}>{order.items.map((item) => { const product = productById(item.productId)!; return <div key={item.key}><span><strong>{item.quantity} × {product.name}</strong><small>{customizationSummary(item) || "Standard preparation"}</small></span><b>{formatMoney(itemUnitPrice(item) * item.quantity)}</b></div>; })}</div><Totals totals={{ subtotal: order.subtotal, discount: order.discount, deliveryFee: order.deliveryFee, tax: order.tax, total: order.total }} promo={order.promo} /><footer><CheckCircle2 /><span><strong>Simulated payment complete</strong><small>No charge was made.</small></span></footer></article></section>; }
...
function Account({ state, setMode, orders, favorites }: { state: OrderingState; setMode: (mode: AccountMode) => void; orders: () => void; favorites: () => void }) { return <section><PageIntro eyebrow="Customer account" title="Your account" text="Use saved details at checkout and review favorites and recent orders." /><div className={styles.accountGrid}><section className={styles.profileCard}><div className={styles.avatar}>NK</div><span className={styles.eyebrow}>Account customer</span><h2>Noura Karim</h2><p>noura@example.test · +961 70 555 014</p><span className={styles.loyalty}><Sparkles /> 1,240 Reward Points</span><button className={styles.primary} type="button" onClick={() => setMode("Account")}>{state.accountMode === "Account" ? "Account selected ✓" : "Use for checkout"}</button></section><div className={styles.accountDetails}><article><MapPin /><div><span>Saved address</span><h2>Home</h2><p>24 Cedar Street, Downtown · Leave at reception</p></div></article><article><Heart /><div><span>Favorites</span><h2>{state.favorites.length} saved items</h2><button type="button" onClick={favorites}>View favorites <ArrowRight /></button></div></article><article><ReceiptText /><div><span>Recent orders</span><h2>{state.orderHistory.length} orders</h2><button type="button" onClick={orders}>View and reorder <ArrowRight /></button></div></article></div></div></section>; }
function ConfirmDialog({ close, confirm }: { close: () => void; confirm: () => void }) {
  useAccessibleDialog(close);
  return <div className={styles.backdrop} onMouseDown={(event) => event.currentTarget === event.target && close()}><section className={styles.confirmDialog} role="dialog" aria-modal="true" aria-labelledby="reset-title"><button className={styles.dialogClose} type="button" aria-label="Close reset dialog" onClick={close}><X /></button><RefreshCcw /><h2 id="reset-title">Reset ordering data?</h2><p>This clears the cart, favorites, branch, customer choice, and simulated orders.</p><div><button className={styles.secondary} type="button" onClick={close}>Cancel</button><button className={styles.danger} type="button" onClick={confirm}>Reset Data</button></div></section></div>;
}
