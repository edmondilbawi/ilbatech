...
function EntitiesView({ state, actor, kind, commit }: { state: DemoState; actor: User; kind: "suppliers" | "destinations"; commit: Commit }) {
  ...
  return <section className={styles.panel}><PanelHeader eyebrow="Master data" title={`${collection.filter((item) => item.active).length} active ${title.toLowerCase()}`} action={<button className={styles.primaryButton} type="button" onClick={() => setEditing(null)}><Plus /> Add {kind === "suppliers" ? "Supplier" : "Destination"}</button>} /><div className={styles.entityGrid}>{collection.map((entity) => <article key={entity.id}><span data-active={entity.active}>{kind === "suppliers" ? <Truck /> : <Building2 />}</span><div><h2 title={entity.name}>{entity.name}</h2><p>{kind === "destinations" ? (entity as Destination).type : "Inbound supplier"}</p></div><StatusBadge status={entity.active ? "Active" : "Inactive"} /><footer><button type="button" onClick={() => setEditing(entity)}><Pencil /> Edit</button><button type="button" data-action={entity.active ? "deactivate" : "activate"} onClick={() => commit(toggleEntityActive(state, kind, entity.id, actor), `${entity.name} ${entity.active ? "deactivated" : "activated"}. Historical records remain unchanged.`)}>{entity.active ? "Deactivate" : "Activate"}</button></footer></article>)}</div>{editing !== undefined && <EntityDialog entity={editing ?? undefined} kind={kind} close={() => setEditing(undefined)} save={(input) => { const result = saveEntity(state, kind, input, actor, editing?.id); if (commit(result, `${input.name} saved.`)) setEditing(undefined); }} />}</section>;
}
